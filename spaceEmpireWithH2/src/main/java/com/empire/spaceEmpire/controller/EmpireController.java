...
import com.empire.spaceEmpire.service.EmpireService;
...
public class EmpireController {
	...
	public String showEmpire(Model model) {
		...
		service.insertEmpire(userEmpire1);
		service.insertEmpire(userEmpire2);
		
		model.addAttribute("empireInfo", service.findAll());
		
		return "spaceEmpire/empireInfo.html";
	}
	
	public Empire getEmpire(Long id) {
		for(Empire empire : service.findAll()) {
			if (empire.getId() == id) {
				return empire;
			}
		}
